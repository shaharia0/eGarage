<?php
session_start();
if(isset($_SESSION['is_adminlogin'])){
    $adminEmail = $_SESSION['adminEmail'];
}else{
    echo "<script>location.href='adminlogin.php'</script>";
}
define('TITLE','Customer Details');
define('PAGE','customerlist');
include ('includes/adminsidebar.php');
include ('../dbconnection.php');
?>
<!-- Customer details table second column start -->
<div class="col-sm-9 col-md-10 mt-4 text-center">
<p class="bg-dark text-white p-2 ">List of Customers</p>
<?php 
 $sql = "SELECT * FROM user_registration_db";
 $result = $conn->query($sql);
 if($result->num_rows > 0){
     echo '<table class="table">';
     echo '<thead>';
     echo '<tr>';
     echo '<th scope="col">Customer ID</th>';
     echo '<th scope="col">Name</th>';
     echo '<th scope="col">Email</th>';
     echo '<th scope="col">Action</th>';
     echo '</tr>';
     echo '</thead>';
     echo '<tbody>';
     while ($row = $result->fetch_assoc()){
         echo '<tr>';
         echo '<td>'.$row["user_login_id"].'</td>';
         echo '<td>'.$row["user_namee"].'</td>';
         echo '<td>'.$row["user_email"].'</td>';
         echo '<td>';
        //  echo '<form action="modifyrequest.php" class="d-inline" method="POST">';
        //  echo '<input type="hidden" name="id" value='.$row["user_login_id"].'>
        //  <button type="submit" class="btn btn-primary mr-3" name="edit" value="Edit">
        //  <i class="fas fa-pen"></i></button>';
        //  echo '</form>';
         echo '<form action="" class="d-inline" method="POST">';
         echo '<input type="hidden" name="id" value='.$row["user_login_id"].'>
         <button type="submit" class="btn btn-secondary mr-3" name="delete" value="Delete">
         <i class="fas fa-trash-alt"></i></button>';
         echo '</form>';
         echo '</td>';
         echo '</tr>';
     }
     echo '</tbody>';
     echo '</table>';
 } else{
     echo '0 Result Found';
 }
?>
</div>
<?php
 if(isset($_REQUEST['delete'])){
     $sql = "DELETE FROM user_registration_db WHERE user_login_id = {$_REQUEST['id']}";
     if($conn->query($sql) == TRUE){
        echo '<meta http-equiv="refresh" content= "0;URL=?deleted"/>';
     }else{
         echo 'Unable to Delete';
     }
 }
?>
<!-- Customer details table second column End -->
</div> <!--end row--->
<!-- add customer button for admin start-->
<!-- <div class="float-right"><a href="insertcustomer.php" class="btn btn-primary">
<i class="fas fa-plus fa-2x"></i></a></div> -->
<!-- add customer button for admin end -->
</div> <!--end container--->
<!-- javascript -->
     <script src="../js/jquery.min.js"></script>
     <script src="../js/popper.min.js"></script>
     <script src="../js/all.min.js"></script>
     <script src="../js/bootstrap.min.js"></script>
</body>
</html>